import { AptosConnectNamespace, type AptosConnectFeature, type AptosConnectOutput, type UserResponse, type Wallet} from '@aptos-labs/wallet-standard';
...
class WalletItem extends LitElement {
  ...
  private _onConnectWallet() {
    this.dispatchEvent(new CustomEvent('wallet-item-click', {
...
export class ConnectWalletModal extends LitElement {
  ...
  protected willUpdate(_changedProperties: PropertyValues): void {
      if (_changedProperties.has('wallets') && (_changedProperties.get('wallets') as Wallet[] || undefined)?.length > 0) {
        this._installedWallets = this.wallets.filter(wallet =>
          default_wallets.some(w => w.name === wallet.name)
        );

        this._uninstalledWallets = default_wallets.filter(wallet =>
          !this.wallets.some(installed => installed.name === wallet.name)
        );
      }
  }
  ...
  private _renderWalletDetail() {
    if (this._needInstallWallet) {
      return html`
        <img class="wallet-logo" src="${this._needInstallWallet.icon}" alt="${this._needInstallWallet.name} logo" />
        ...
        <a class="primary-button" href="${this._needInstallWallet.url}" target="_blank">Get ${this._needInstallWallet.name}</a>
      `;
    }
          ...
          <div class="left-panel">
            ...
            <div class="wallet-list">
              <div class="list-section">
                ...
                ${this._installedWallets.map(wallet => html`<wallet-item .name=${wallet.name} .icon=${wallet.icon} @click=${() => this._onWalletItemClick({wallet,name: wallet.name, isInstalled: true})}></wallet-item>`)}
              </div>
              <div class="list-section">
                <h3 class="section-title">Popular</h3>
                ${this._uninstalledWallets.map(wallet => html`<wallet-item .name=${wallet.name} .icon=${wallet.icon} @click=${() => this._onWalletItemClick({name: wallet.name, isInstalled: false})}></wallet-item>`)}
              </div>
            </div>
          </div>
      ...
      if (wallet) {
            ...
            (wallet.features as AptosConnectFeature)[AptosConnectNamespace].connect()
              .then((userResponse: UserResponse<AptosConnectOutput>) => {
                ...
                this.dispatchEvent(new CustomEvent<WalletConnectedEvent>('wallet-connected', {
                  detail: { address: userAddress.toString(), publicKey: userPublicKey.toString(), ansName: userAnsName, wallet },
                  bubbles: true,
                  composed: true
                }));
                this.closeModal();
